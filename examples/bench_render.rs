...
use termpdb::render::{
    Camera, ColorScheme, Framebuffer, Lighting, LodMode, RenderContext, RenderMode, RibbonPrimitive,
    Visibility, build_render_cache, build_ribbon_geometry, render_structure, render_structure_ctx,
};
...
fn build(n_per_chain: u32, chain_ids: &[&str]) -> Structure {
    ...
    for (ci, cid) in chain_ids.iter().copied().enumerate() {
        ...
        for i in 0..n_per_chain {
            ...
            let atom = Atom::new(
                0, (i + 1) as i32, "CA", elem, Vec3::new(x, y, z),
                20.0, "ALA", (i + 1) as i32, cid, false,
            );
...
fn bench_fresh(structure: &Structure, mode: RenderMode, scheme: ColorScheme, lod: LodMode, w: usize, h: usize, zoom: f32) -> (std::time::Duration, usize) {
    let camera = fit_camera(structure, w, h, zoom);
    let lighting = Lighting::default();
    let mut fb = Framebuffer::new(w, h * 2);
    fb.clear((0, 0, 0));
    render_structure(structure, mode, scheme, &camera, &mut fb, &lighting, Visibility::default(), lod);
    let iters = 30;
...
fn bench_cached(structure: &Structure, mode: RenderMode, scheme: ColorScheme, lod: LodMode, w: usize, h: usize, zoom: f32) -> (std::time::Duration, usize) {
    let camera = fit_camera(structure, w, h, zoom);
    let lighting = Lighting::default();
    let visibility = Visibility::default();
    let (colors, visible, com, radius, max_vdw) = build_render_cache(structure, scheme, visibility, lod);
    ...
    let ctx = RenderContext {
        structure, camera: &camera, mats, lighting: &lighting, visibility, lod: level,
        colors: &colors, visible: &visible, com, radius, max_vdw,
...
fn fps(d: std::time::Duration) -> f32 { 1.0 / d.as_secs_f32().max(1e-9) }

fn main() {
    for &n in &[5_000u32, 20_000u32] {
        let chains = if n <= 5_000 { vec!["A"] } else { vec!["A", "B", "C", "D"] };
        let structure = build(n, &chains);
        println!("\n=== {} atoms (CPK): fresh vs cached (orbit/spin) ===", structure.atom_count());
        for &mode in &[RenderMode::Vdw, RenderMode::BallAndStick, RenderMode::Ribbon, RenderMode::Trace] {
            let (df, drawn) = bench_fresh(&structure, mode, ColorScheme::Cpk, LodMode::Full, 120, 60, 1.0);
            let (dc, _) = bench_cached(&structure, mode, ColorScheme::Cpk, LodMode::Full, 120, 60, 1.0);
            println!(
                "{:13}  fresh {:7.3} ms ({:5.0} fps) | cached {:7.3} ms ({:5.0} fps)  {:.1}x  drawn={}",
                format!("{:?}", mode), ms(df), fps(df), ms(dc), fps(dc), ms(df) / ms(dc).max(1e-9), drawn
            );
        }
    }
    ...
    for &n in &[5_000u32, 20_000u32] {
        let chains = if n <= 5_000 { vec!["A"] } else { vec!["A", "B", "C", "D"] };
        let structure = build(n, &chains);
        let (d, drawn) = bench_cached(&structure, RenderMode::Vdw, ColorScheme::BFactor, LodMode::Full, 120, 60, 1.0);
        println!("  {:>6} atoms VDW/BFactor  {:7.3} ms/frame  ({:5.0} fps)  drawn={}",
            structure.atom_count(), ms(d), fps(d), drawn);
    }

    println!("\n=== Zoomed-in VDW (heavy overdraw, 1000 atoms, 160x80) ===");
    let structure = build(1_000, &["A"]);
    for &(label, zoom) in &[("1x", 1.0f32), ("4x", 4.0), ("10x", 10.0)] {
        let (d, drawn) = bench_cached(&structure, RenderMode::Vdw, ColorScheme::Cpk, LodMode::Full, 160, 80, zoom);
        println!("  zoom {:3}  {:7.3} ms/frame  ({:5.0} fps)  drawn={}/{}",
            label, ms(d), fps(d), drawn, 160 * 80 * 2);
    }
}
