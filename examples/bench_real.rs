...
fn main() {
    let path = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "/tmp/4egk.pdb".to_string());
    ...
    println!(
        "ensure_bonds:       {:7.2} ms  (bonds={})",
        ms(t.elapsed()),
        structure.bonds().len()
    );

    println!(
        "atoms={} residues={} chains={} models={}",
    ...
    println!(
        "build_ribbon_geom: {:7.2} ms  (prims={})",
        ms(t.elapsed()),
        rg.len()
    );
    ...
    for &mode in &[
        ...
        let rg_ref = if mode == RenderMode::Ribbon {
            Some(&rg[..])
        } else {
            None
        };
    ...
    for _ in 0..iters {
        fb.clear((0, 0, 0));
        render_structure_ctx(&ctx, RenderMode::Vdw, &mut fb);
    }
    let d = t.elapsed() / iters;
    println!(
        "VDW/BFactor       {:7.3} ms/frame  ({:6.1} fps)",
        ms(d),
        1.0 / d.as_secs_f32().max(1e-9)
    );
}
