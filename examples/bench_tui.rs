...
fn main() {
    ...
    for &(mode, label) in &[
        ...
        for _ in 0..10 { app.tick(0.016); let _ = terminal.draw(|f| app.render_ui(f)); }
        let iters = 300;
        let t = std::time::Instant::now();
        for _ in 0..iters { app.tick(0.016); let _ = terminal.draw(|f| app.render_ui(f)); }
        let d = t.elapsed() / iters;
        println!("{:11} TUI frame (spin, 160x100): {:6.3} ms  ({:5.0} fps)", label, d.as_secs_f32()*1000.0, 1.0/d.as_secs_f32());
    }
    ...
    let mut app = App::new(structure.clone(), RenderMode::Ribbon, ColorScheme::Cpk, false);
    let backend = TestBackend::new(160, 100);
    let mut terminal = Terminal::new(backend).unwrap();
    ...
    let d = t.elapsed() / iters;
    println!("{:11} TUI idle (gated, 160x100): {:6.4} ms/iter  (draws={}/{} -> {:.0}% fewer draws vs 60fps)", "Ribbon", d.as_secs_f32()*1000.0, draws, iters, 100.0 * (1.0 - draws as f32 / iters as f32));
}
